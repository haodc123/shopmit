...
public class ListMemberAdapter extends BaseAdapter {
	...
	private TextView tv_dl_status;
	private String curStaffID; // current staff of dialog staff openning
		...
		if (m.getIsDisplay() == 0) {
			...
			convertView.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					// TODO Auto-generated method stub
					if (!Variables.curFrg.equalsIgnoreCase(Constants.TAG_FRG_WHOLIKE))
	...
	protected void displayDialog(final String id, final String name, final String email, String joindate, String url) {
		// TODO Auto-generated method stub
		curStaffID = id;
		IOManager.instance(context).check_user_online(Integer.parseInt(curStaffID));
		...
		tv_dl_status = (TextView)mDialogFile.findViewById(R.id.tv_dl_status);
		...
		tv_dl_joindate.setText("Joined "+Functions.getFriendlyDateCreated(today, joindate, Constants.DATETIME_FORMAT));

		new loadRemoteIMG(img_dl_avatar).execute(Constants.FOLDER_AVATAR+url);
	...
	public void setStatusStaff(String status, String user_id) {
		if (!user_id.equalsIgnoreCase(curStaffID))
			return;
		tv_dl_status.setText(status+"line");
		switch (status) {
			case "off":
				tv_dl_status.setTextColor(context.getResources().getColor(R.color.gray_global));
				break;
			case "on":
				tv_dl_status.setTextColor(context.getResources().getColor(R.color.green_global));
				break;
		}
	}
	...
	public void goCall(String id, String name, String email) {
		if (!Functions.hasConnection(context)) {
			Functions.toastString(context.getString(R.string.alert_no_internet), context);
			return;
		}
		Variables.prevFrg = Variables.curFrg;
		Variables.curFrg = Constants.TAG_FRG_CALL;
		((MainActivity)context).setDisplayCall(1, id, name, email);
	}
	public void goVideo(String id, String name, String email) {
		...
		((MainActivity)context).setDisplayCall(2, id, name, email);
	}
	...
	public static class ListMemberData {
        ...
        public ListMemberData(String mb_id, String mb_uname, String mb_fname, String mb_lname,
							  String url_avatar, String mb_email, String mb_joindate, int isDisplay) {
            this.mb_id = mb_id;
